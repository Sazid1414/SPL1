...
/* Function to send the client's name to the server */
void send_name_to_server(int sockfd, const char *name)
{
    if (write(sockfd, name, 32) < 0)
    {
        perror("ERROR: write to descriptor failed");
    }
}
...
int register_user()
{
    ...
    else
    {
        ...
        send_name_to_server(sockfd, new_user.name);
...
//         // Send the name to the server
//         send_name_to_server(sockfd, user.name);
...
int login_user()
{
    ...
    struct User user; // Define user object here
    ...
    if (authenticate_user("user.txt", email, password))
    {
        ...
        // Send the name to the server
        send_name_to_server(sockfd, user.name); // Use user.name here
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        if (recv(cli->sockfd, cli->name, 32, 0) <= 0)
        {
            printf("Couldn't get the client's name\n");
            close(connfd);
            free(cli);
            continue;
        }
