...
void ShiftRowForEncryption(unsigned char *messages)
{
    ...
    temporary[3] = messages[15];

    temporary[4] = messages[4];
    temporary[5] = messages[9];
    temporary[6] = messages[14];
    temporary[7] = messages[3];

    temporary[8] = messages[8];
    temporary[9] = messages[13];
    temporary[10] = messages[2];
    temporary[11] = messages[7];
...
void mixColumnForEncryption(unsigned char *messages)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        if (i % 4 == 0)
        {
            temporary[i] = multiplicationTableFor2[messages[i]] ^ multiplicationTableFor3[messages[i + 1]] ^ messages[i + 2] ^ messages[i + 3];
        }
        if (i % 4 == 1)
        {
            temporary[i] = multiplicationTableFor2[messages[i]] ^ multiplicationTableFor3[messages[i + 1]] ^ messages[i + 2] ^ messages[i - 1];
        }
        if (i % 4 == 2)
        {
            temporary[i] = multiplicationTableFor2[messages[i]] ^ multiplicationTableFor3[messages[i + 1]] ^ messages[i - 1] ^ messages[i - 2];
        }
        if (i % 4 == 3)
        {
...
int diffie_hellman(int p, int g, int a, int b) {
    int A, B, shared_secret_A, shared_secret_B;
...
void BlockByBlockEncryption(unsigned char* EncryptedMessage,int LengthOfExtendedMessage,unsigned char* key)
{
    ...
    CipherText(EncryptedMessage,sizeof(EncryptedMessage));
}


void Decryption(unsigned char* EncryptedMessage,int LengthOfExtendedMessage,int LengthOfMessage)
{
